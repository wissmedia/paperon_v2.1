...
function qbankArrayRender(data, isPreview) {
  const { id, tipe, body, opsiy, opsix, sl, label, useWajib } = data
  let isWajib = ''
  let setPreview = ''
  let hiddenInput = ''
  switch (tipe) {
    case 'shortText':
      ...
      return `
        ...
        <div class="bungkus-content edit">
          ...
          <input type="text" class="text tanya" name="${id}" id="shortText" placeholder="Answer here" ${isWajib} ${setPreview}>
        </div>
          ${hiddenInput}
      `
    case 'longText':
      ...
      return `
        ...
        <div class="bungkus-content edit">
          <p class="judul">Answer${isWajib ? '*' : ''} : </p>
          <textarea name="${id}" id="longText" cols="30" rows="10" placeholder="Answer here" ${isWajib} ${setPreview}></textarea>
        </div>
        ${hiddenInput}
      `
    case 'radio':
      ...
      outputR = opsiy.map((opsi, i) => {
        return `
          <p>
            <input type="radio" name="${id}" id="${opsi}" value="${opsi}" ${isWajib} ${setPreview}>
      ...
      return `
        ...
        <div class="bungkus-content edit">
          ...
          ${isWajib ? '' : `<input type="radio" hidden name="${id}" value="" checked ${isWajib}>`}
          ${outputR}
          ${hiddenInput}
    ...
    case 'checkBox':
      ...
      outputCB = opsiy.map((opsi, i) => {
        return `
          <p>
            <input type="checkBox" name="${id}" id="${opsi}" value="${opsi}" ${setPreview}>
            <label for="${opsi}">${opsi}</label>
          </p>
          `
      }).join('')
    ...
    case 'dropDown':
      ...
      return `
        ...
        <div class="bungkus-content">
          <p class="judul">Answer${isWajib ? '*' : ''} : </p>
          <select name="${id}" id="${id}" class="dropdown" ${isWajib}>
    ...
    case 'date':
      ...
      return `
        ...
        <div class="bungkus-content">
          ...
          <p class="judul">Answer${isWajib ? '*' : ''} : </p>
            <input type="date" name="${id}" id="${id}" ${isWajib}>
          </div>
        </div>  
        ${hiddenInput}
      `
    case 'time':
      ...
      return `
        ...
        <div class="bungkus-content">
          <div class="form_group">
            <p class="judul">Answer${isWajib ? '*' : ''} : </p>
            <input type="time" name="${id}" id="${id}" ${isWajib}>
          </div>
        </div>
        ${hiddenInput}
      `
    case 'linearScale':
      ...
      function fun2() {
        ...
        for (let i = sl[0]; i <= last; i++) {
          str2 += `
            <td class="text-center">
              <input type="radio" name="${id}" value="${i}" ${isWajib}/>
            </td>
          ` }
        return str2
      }
    ...
    case 'dateTime':
      ...
      return `
        ...
        <div class="bungkus-content">
          <div class="form_group">
            <p class="judul">Answer${isWajib ? '*' : ''} : </p>
            <input type="datetime-local" name="${id}" id="${id}" ${isWajib}>
          </div>
        </div>
        ${hiddenInput}
      `
